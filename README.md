...
Below is a visual diagram (Mermaid) showing how the main services, containers, and code files interact in this messaging app. It covers REST and WebSocket flows, and how the main Python files call each other for real-time messaging.
...
flowchart TD
  ...
  subgraph Messaging_Service
    MS[ASGI Server (Daphne/Uvicorn)]
    API[chat_api/views.py (REST API)]
    CONS[chat/consumers.py (WebSocket)]
    MW[chat/middleware.py (JWT Auth)]
    AUTHC[chat/auth_client.py (Auth API client)]
    MODELS[chat/models.py (ChatRoom, Message)]
    ROUTE[chat/routing.py (WS URL patterns)]
    ENTRY[entrypoint.sh (migrations)]
  end
  subgraph Auth_Service
    AUTHAPI[Tutoring_API (Django REST)]
    AUTHDB[(auth_db: Postgres)]
  end
  subgraph Data
    MSGDB[(messaging_db: Postgres)]
    REDIS[(Redis: Channel Layer)]
  end

  %% External client
  CLIENT[Frontend / Postman / wscat]
  CLIENT -- HTTP/WS --> T
  T -- HTTP --> MS
  T -- WS --> MS

  %% REST flow
  CLIENT -- REST /api/chat/rooms/ --> API
  API -- validate token --> MW
  MW -- decode JWT --> API
  API -- fetch profile --> AUTHC
  AUTHC -- HTTP --> AUTHAPI
  AUTHAPI -- DB --> AUTHDB
  API -- create/get room --> MODELS
  API -- DB --> MSGDB

  %% WebSocket flow
  CLIENT -- WS /ws/chat/{room_id}/ --> CONS
  CONS -- validate token --> MW
  MW -- decode JWT --> CONS
  CONS -- verify user in room --> MODELS
  CONS -- create message --> MODELS
  CONS -- save message --> MSGDB
  CONS -- broadcast message --> REDIS
  REDIS -- pub/sub --> MS
  CONS -- fetch profile (optional) --> AUTHC
  AUTHC -- HTTP --> AUTHAPI
  ...
  class MS,API,CONS,MW,AUTHC,MODELS,ROUTE,ENTRY code;
  class AUTHAPI,AUTHDB service;
  class MSGDB,REDIS data;
```
